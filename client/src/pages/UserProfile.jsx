...
const UserProfile = () => {
    ...
    const fetchUserProfile = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BASE_URL_BACKEND}/Profile/${userId}`
        );
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-12 px-4 sm:px-6 lg:px-8 pt-22">
          ...
          <div className="p-6 sm:p-8">
            <div className="flex items-center space-x-4 mb-2">
              <div className="bg-blue-100 dark:bg-blue-900 p-3 rounded-full">
                <FiUser className="text-blue-600 dark:text-blue-300 text-2xl" />
              </div>
              <h2 className="text-2xl font-bold text-gray-800 dark:text-white">
                {data.user.name || "Uploader"}
              </h2>
            </div>
          </div>
        </div>
          ...
          <div className="p-6 sm:p-8">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3">
                ...
                <div className="flex items-center space-x-2 mt-1 sm:mt-0">
                  ...
                  <span className="bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 text-xs font-medium px-2.5 py-0.5 rounded-full">
                    {data.notes?.reduce(
                      (sum, note) => sum + (note.upvotesCount || 0),
                      0
                    )}{" "}
                    Upvotes
                  </span>
            ...
            {data.notes && data.notes.length > 0 ? (
                ...
                {data.notes.map((note) => (
                  ...
                  >
                    ...
                    {note.upvotesCount > 0 && (
                      <p className="text-sm text-gray-600 dark:text-gray-300">
                        Upvotes:{" "}
                        <span className="font-medium">{note.upvotesCount}</span>
                      </p>
                    )}
                    {note.fileurl && (
                      <button
              ...
              <div className="text-center py-8">
                <FiBook className="mx-auto text-3xl text-gray-400 dark:text-gray-600 mb-2" />
          ...
          <div className="p-6 sm:p-8">
            <div className="flex items-center space-x-3 mb-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3">
                ...
                <div className="flex items-center space-x-2 mt-1 sm:mt-0">
                  <span className="bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200 text-xs font-medium px-2.5 py-0.5 rounded-full">
                    {data.pyqs?.length || 0} Files
                  </span>
                  <span className="bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 text-xs font-medium px-2.5 py-0.5 rounded-full">
                    {data.pyqs?.reduce(
                      (sum, pyq) => sum + (pyq.upvotesCount || 0),
                      0
                    )}{" "}
                    Upvotes
                  </span>
            ...
            {data.pyqs && data.pyqs.length > 0 ? (
                ...
                {data.pyqs.map((pyq) => (
                  ...
                  >
                    ...
                    {pyq.upvotesCount > 0 && (
                      <p className="text-sm text-gray-600 dark:text-gray-300">
                        Upvotes:{" "}
                        <span className="font-medium">{pyq.upvotesCount}</span>
                      </p>
                    )}
                    {pyq.fileurl && (
                      <button
              ...
              <div className="text-center py-8">
                <FiFileText className="mx-auto text-3xl text-gray-400 dark:text-gray-600 mb-2" />
                <p className="text-gray-500 dark:text-gray-400">
