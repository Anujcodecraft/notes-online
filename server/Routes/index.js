...
Router.post("/signup", checkEmail, async (req, res) => {
  try {
    ...
    if (!user)
      return res.status(NO_CONTENT).json({ message: "Service unavailable" });
...
Router.post("/login", checkEmail, async (req, res) => {
  try {
    ...
    if (userone.provider !== "local")
      return res
        .status(404)
        .json({ message: "Please Try sign in using google" });
...
Router.get("/userdetails", mainmiddleware, async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(CONFLICT)
        .json({ message: "No user with the current id" });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (UUser.role !== "uploader") {
        return res
          .status(403)
          .json({ error: "Approval required for uploading" });
      }
      ...
      res
        .status(400)
        .json({ error: "Note upload failed", details: err.message });
    }
  }
);
...
Router.get("/notes", mainmiddleware, async (req, res) => {
  try {
    ...
    const notes = await notesModel
      ...
      .exec();
    const documentsNum = await notesModel.countDocuments();

    if (documentsNum <= (page + 1) * 6) isLastPage = true;
    res.status(200).json({ notes, isLastPage });
  } catch (err) {
    console.log(err);
    res
      .status(500)
...
Router.get("/pyqs", async (req, res) => {
  try {
    ...
    const documentsNum = await pyqModel.countDocuments();
    if (documentsNum <= (page + 1) * 6) isLastPage = true;

    res.status(200).json({ pyqs, isLastPage });
  } catch (err) {
...
Router.post("/notes/:id/upvote", mainmiddleware, async (req, res) => {
  try {
    ...
    if (alreadyUpvoted) {
      ...
      return res
        .status(400)
        .json({ error: "You have already upvoted this note" });
    }
    ...
    return res
      .status(500)
      .json({ error: "Failed to upvote note", details: err.message });
  }
});
...
Router.delete("/notes/:id/upvote", mainmiddleware, async (req, res) => {
  try {
    ...
    const upvoteIndex = note.upvotes.findIndex(
      (id) => id.toString() === user._id.toString()
    );
    if (upvoteIndex === -1) {
      ...
      return res
        .status(400)
        .json({ error: "You haven't upvoted this note yet" });
    }
    ...
    return res
      .status(500)
      .json({ error: "Failed to remove upvote", details: err.message });
  }
});
...
Router.get("/my-pyqs", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch pyqs", details: err.message });
  }
});
...
Router.delete("/delete-note/:noteId", async (req, res) => {
  try {
    ...
    await userModel.updateOne({ notes: noteId }, { $pull: { notes: noteId } });
...
Router.post("/pyqs/:id/upvote", mainmiddleware, async (req, res) => {
  try {
    ...
    if (pyq.upvotes.includes(user._id)) {
      return res
        .status(400)
        .json({ error: "You have already upvoted this PYQ" });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to upvote PYQ", details: err.message });
  }
});
...
Router.delete("/delete-pyq/:pyqId", async (req, res) => {
  try {
    ...
    // Remove note reference from user's notes array
    await pyqModel.updateOne({ pyqs: pyqId }, { $pull: { pyqs: pyqId } });
    ...
    res
      .status(500)
      .json({ error: "Failed to delete pyq", details: err.message });
  }
});
